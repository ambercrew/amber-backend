...
namespace Brainy.Infrastructure.Tests.Users.Repositories;
...
public class UserRepositoryTests : RepositoryTestBase
{
    ...
    {
        _userRepository = new(BrainyContext);
    }
    ...
    public async Task DeleteUserByIdAsync_UserWithSyncedEntities_DeletedUserAndSyncedEntities()
    {
        ...
        await BrainyContext.SyncedEntities.AddAsync(CreateSyncedEntity(user1));
        await BrainyContext.SyncedEntities.AddAsync(CreateSyncedEntity(user1));

        var user2 = UserTestUtils.CreateUser("user2");
        await _userRepository.AddAsync(user2);
        await BrainyContext.SyncedEntities.AddAsync(CreateSyncedEntity(user2));
        ...
        BrainyContext.SyncedEntities.Single().UserId.Should().Be(user2.Id);
    }
    ...
    public async Task GetInactiveUsersAsync_UserWithSyncedEntityLastSyncedBeforeCutoff_ReturnsUser()
    {
        ...
        await _userRepository.AddAsync(user);
        await BrainyContext.SyncedEntities.AddAsync(
    ...
    public async Task GetInactiveUsersAsync_UserWithSyncedEntityLastSyncedExactlyAtCutoff_ReturnsEmpty()
    {
        ...
        var user = UserTestUtils.CreateUser("user1");
        await _userRepository.AddAsync(user);
        await BrainyContext.SyncedEntities.AddAsync(CreateSyncedEntity(user, cutoffDate));
        await _userRepository.SaveChangesAsync();
